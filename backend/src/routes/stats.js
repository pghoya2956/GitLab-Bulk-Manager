...
// Helper: Recursively count subgroups
async function countSubgroupsHelper(baseURL, token, groupId, depth, hierarchy) {
  try {
    ...
    for (const subgroup of response.data) {
      await countSubgroupsHelper(baseURL, token, subgroup.id, depth + 1, hierarchy);
    }
  } catch (error) {
...
router.get('/groups/hierarchy', async (req, res) => {
  try {
    ...
    for (const group of topLevelResponse.data.slice(0, 10)) { // Limit to prevent timeout
      await countSubgroupsHelper(baseURL, token, group.id, 1, hierarchy);
